...
name: golang
description: |
  Read this BEFORE starting to work on a Golang (Go) codebase.
  Make sure the follow these instruction as close as possible, especially when it comes to documentation lookup.
...
Alternatively you can also use `go doc`.
The high level syntax is:

1. Package level documentation: `go doc <pkg>`
2. Documentation for a symbol: `go doc <sym>` optionally with a method or field: `<sym>[.<methodOrField>]`
3. Documentation for a symbol in a specific package: `go doc [<pkg>.]<sym>[.<methodOrField>]` or `go doc [<pkg>.][<sym>.]<methodOrField>`
4. To get the full source of a symbol (e.g. a function) add the `-src` argument: `go doc -src <additonal args>`
